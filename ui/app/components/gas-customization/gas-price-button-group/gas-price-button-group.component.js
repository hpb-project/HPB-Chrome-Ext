...
export default class GasPriceButtonGroup extends Component {
  ...
  renderButtonContent ({
    ...
    feeInPrimaryCurrency = feeInPrimaryCurrency.replace(/ETH/,"HPB");
    return (<div>
  ...
  render () {
    const {
      gasButtonInfo,
      //leo
      gasButtonInfoAnother = [{
        feeInPrimaryCurrency: "0.00042 ETH",
        feeInSecondaryCurrency: "$0.00",
        labelKey: "fastest",
    ...
    return (
      !buttonDataLoading
        ...
        >
          { gasButtonInfoAnother.map((obj, index) => this.renderButton(obj, buttonPropsAndFlags, index)) }
        </ButtonGroup>
